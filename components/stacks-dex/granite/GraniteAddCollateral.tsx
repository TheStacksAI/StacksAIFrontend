...
export default function GraniteAddCollateral({ data, isLoading }: GraniteAddCollateralProps) {
  ...
  const collateralToken = transaction.functionArgs[0]?.value || "";
  const amount = transaction.functionArgs[1]?.value || "0";
  const asset = "sBTC"; // Granite add collateral is typically sBTC
  ...
  return (
    <TransactionWrapper transactionData={transactionData} network={transaction.network === "mainnet" ? "mainnet" : "testnet"} buttonText="Sign & Add Collateral" buttonGradient="from-slate-600 to-gray-600 hover:from-slate-700 hover:to-gray-700">
      <Card className="w-full bg-gradient-to-br from-slate-500/10 to-gray-500/10 border-slate-500/20">
        <CardHeader><div className="flex items-center gap-2"><PlusCircle className="w-6 h-6 text-slate-400" /><CardTitle className="text-xl">Add Collateral</CardTitle></div><CardDescription className="text-zinc-300">Add {displayAmount} {asset} to {marketName}</CardDescription></CardHeader>
        <CardContent className="space-y-6">
          <div className="bg-slate-500/10 border border-slate-500/20 rounded-lg p-6 text-center"><span className="text-sm text-zinc-400 block mb-2">Additional Collateral</span><p className="text-4xl font-bold text-slate-400">{displayAmount}</p><p className="text-xs text-zinc-500 mt-2">{asset}</p></div>
